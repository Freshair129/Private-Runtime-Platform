...
- **Contract binding**: `api/routes.py` declares every client operation with the contract's `operationId` and binds JSON bodies, parameters and success responses to the generated models; `tests/contracts/test_openapi_conformance.py` checks the inventory and compares each operation's schemas in `app.openapi()` with `contracts/openapi/prp-client.yaml` after normalization. The worker does the same against `prp-worker.yaml`. The two multipart operations stay unbound until the upload path exists. Change the YAML first, run `gen_models.py`, then adjust the routes. Use `pattern: '^https://'` rather than `format: uri` for URL fields.
