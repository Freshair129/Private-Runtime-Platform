#!/usr/bin/env python3
"""Rebuild reading HTML views from the living docs/ tree (writes docs/*.html). Not application code.
...
def build_atlas():
 cat=json.loads((ROOT/'diagrams/catalog.json').read_text(encoding='utf-8'));cards=[]
 for d in cat:
  d['svg']=(ROOT/f'diagrams/svg/{d["id"]}.svg').read_text(encoding='utf-8');d['svg']=re.sub(r'<\?xml.*?\?>|<!DOCTYPE.*?\]>|<!DOCTYPE[^>]+>','',d['svg'],flags=re.S)
